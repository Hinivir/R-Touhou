...
Server::Server(const std::string &ip, int const port) : server_socket(io_service, udp::endpoint(udp::v4(), port))
{
    try {
        server_socket.close();
...
Server::~Server(void) { closeServer(); }

void Server::closeServer(void) { server_socket.close(); }
...
const std::map<std::string,
    std::function<void(Server &, const udp::endpoint &, const std::array<char, 2048> &, size_t)>>
    Server::serverCommandHandler = {{"connect\n", &Server::handleConnect}, {"disconnect\n", &Server::handleDisconnect},
        {"ready\n", &Server::handleReady}};

void Server::handleConnect(
    const udp::endpoint &client_endpoint, const std::array<char, 2048> &buffer, size_t bytes_received)
{
    std::string message(buffer.data(), bytes_received);
    if (message == "connect\n") {
        if (std::find(alreadyConnectedClients.begin(), alreadyConnectedClients.end(), client_endpoint) ==
            alreadyConnectedClients.end()) {
            if (playerCount < maxPlayers) {
                ...
                std::string newUserMessage = "New user connected: " + client_endpoint.address().to_string() + ":" +
                ...
                std::cout << playerMessage << playerCount + 1 << ": " << client_endpoint.address() << ":"
                ...
                try {
                    ...
                    std::cerr << "Error sending full message to client " << client_endpoint.address() << ":"
                              << client_endpoint.port() << ": " << e.what() << std::endl;
                }
            }
        }
    ...
    const udp::endpoint &client_endpoint, const std::array<char, 2048> &buffer, size_t bytes_received)
{
    ...
    if (it != connectedClients.end()) {
        ...
        auto alreadyConnectedIt =
            std::find(alreadyConnectedClients.begin(), alreadyConnectedClients.end(), client_endpoint);
...
void Server::notifyGameReady()
{
    for (const auto &client : connectedClients) {
        try {
            ...
            std::cerr << "Error sending game ready message to client " << client.address() << ":" << client.port()
                      << ": " << e.what() << std::endl;
        }
    }
}
    ...
    for (const auto &client : connectedClients) {
        if (client != sender) {
            try {
                ...
                std::cerr << "Error sending message to client " << client.address() << ":" << client.port() << ": "
                          << e.what() << std::endl;
            }
        }
    }
